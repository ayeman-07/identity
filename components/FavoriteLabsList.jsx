...
export default function FavoriteLabsList({ favoriteLabs, recommendedLabs, onFavoriteChange }) {
  ...
  const renderLabCard = (lab, isFavorite = false) => {
    ...
    return (
      <div key={lab.id} className="glass-card p-4 hover:bg-white/10 transition-colors">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center">
                <h3 className="font-medium text-gray-100">{lab.name}</h3>
                {isFavorite && (
                  <svg className="w-4 h-4 ml-2 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
              ...
              <button
                ...
                className={`p-1 rounded-full hover:bg-white/10 transition-colors ${
                  isCurrentlyFavorite ? 'text-red-300' : 'text-gray-400'
                }`}
            ...
            {lab.specialties && lab.specialties.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {lab.specialties.slice(0, 3).map((specialty, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-200"
                  >
                    {specialty}
                  </span>
                ))}
        ...
        <div className="flex space-x-2">
          <Link
            ...
            className="flex-1 inline-flex justify-center items-center text-sm leading-4 font-medium rounded-md btn-gradient px-3 py-2"
          >
            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {favoriteLabs && favoriteLabs.length > 0 && (
    ...
    <h3 className="text-lg font-medium text-gray-100">Favorite Labs</h3>
            <Link
              href="/clinic/labs"
  ...
  {recommendedLabs && recommendedLabs.length > 0 && (
    ...
    {(!favoriteLabs || favoriteLabs.length === 0) && (!recommendedLabs || recommendedLabs.length === 0) && (
      ...
      <h3 className="mt-2 text-sm font-medium text-gray-100">No labs found</h3>
      <p className="mt-1 text-sm text-gray-400">Get started by exploring available labs.</p>
          <div className="mt-6">
            <Link
              href="/clinic/labs"
