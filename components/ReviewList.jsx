...
export default function ReviewList({ labId, className = '' }) {
  ...
  if (error) {
    return (
      <div className={`text-red-300 text-center py-4 ${className}`}>
        {error}
      </div>
    );
  }
            ...
            {[5, 4, 3, 2, 1].map((rating) => {
              ...
              return (
                <div key={rating} className="flex items-center space-x-3 text-sm">
                  <div className="flex items-center space-x-1 w-12 text-gray-300">
                    <span>{rating}</span>
        ...
        {reviews.map((review) => (
          <div key={review.id} className="glass-card p-4">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-indigo-500/10 text-indigo-300 rounded-full flex items-center justify-center">
